...
```text
CONNECT huanyuxingqiu.fun:443 HTTP/1.1
```

如果代理只是普通隧道代理，它只能知道客户端要连 `huanyuxingqiu.fun:443`。随后客户端和远端服务器之间会建立 TLS 加密通道，代理只负责转发加密字节流。

在这种模式下，代理看不到这些信息：

- 完整 URL：`https://huanyuxingqiu.fun/api/v1/member/profile?token=local-test`
...
```text
^https?://(?:huanyuxingqiu\.(?:fun|vip|life|com|top|cc|app|xyz|site)|daoyu\.(?:fan|com|top)|hyxq\w*\.\w+)/(?!.*\.(?:png|jpe?g|gif|webp|svg|ico|css|js|woff2?|ttf|eot|mp[34]|zip|rar)(?:\?|$))(.*)
```
...
```bash
curl -sk --proxy http://127.0.0.1:28880 "https://huanyuxingqiu.fun/api/v1/member/profile?token=local-test"
```
...
```text
^https?://
(?:huanyuxingqiu\.(?:fun|vip|life|com|top|cc|app|xyz|site)|daoyu\.(?:fan|com|top)|hyxq\w*\.\w+)
/
...
```text
https://huanyuxingqiu.fun/api/v1/member/profile?token=local-test
```
...
```text
https://huanyuxingqiu.fun/app.js
```

按照原 Surge 规则，`.js` 应该被负向前瞻排除，不应该改写到 mock origin。

但如果只是“不改写”，mitmproxy 默认会继续尝试访问真实 `huanyuxingqiu.fun`。为了让测试全程留在本机，`rewrite_addon.py` 默认会把“属于配置域名但没有被改写”的请求返回本地 `599`：
...
```bash
LOCAL_MITM_BLOCK_UNREWRITTEN=0 mitmdump --listen-host 127.0.0.1 --listen-port 28880 --set ssl_insecure=true -s rewrite_addon.py
```
...
2. 安装 `mitmproxy`。
3. 运行单元测试。
4. 启动 mitmproxy：`127.0.0.1:28880`。
5. 发送正例请求：

   ```bash
   curl -skL --proxy http://127.0.0.1:28880 "https://huanyuxingqiu.fun/4687.html"
   ```
...
7. 发送负例请求：
   ...
   ```bash
   curl -sk --proxy http://127.0.0.1:28880 "https://huanyuxingqiu.fun/app.js"
   ```
...
这条路径最接近 Surge：浏览器仍然访问原始地址，例如 `https://huanyuxingqiu.fun/4687.html`，本地 mitmproxy 负责把命中的请求改写到真实 Worker。
...
```text
https://huanyuxingqiu.fun/4687.html
```

也可以指定其他路径：

```bash
./open_chrome_with_mitm.sh "https://huanyuxingqiu.fun/57374.html"
```
...
这套脚本不会修改系统代理设置。它通过 `--user-data-dir=.chrome-mitm-profile` 打开一个隔离的 Chrome profile，并通过 `--proxy-server=http://127.0.0.1:28880` 让这个窗口走本地代理。代理日志位于：
...
```text
https://huanyuxingqiu.fun/3199.html
```
...
`start_console.sh` 默认会启动本地 MITM 代理 `http://127.0.0.1:28880`，并在按 `Ctrl-C` 退出时一起关闭代理、后端和前端。如果只想启动控制台、不管理代理：
...
抓文章页 -> 通过 <proxy-gateway-host>:7890 访问 huanyuxingqiu.fun
...
```bash
mitmdump --listen-host 127.0.0.1 --listen-port 28880 --set ssl_insecure=true -s rewrite_addon.py
```
...
```bash
curl -skL --proxy http://127.0.0.1:28880 "https://huanyuxingqiu.fun/57517.html" -o /tmp/huanyuxingqiu-57517.html
.venv/bin/python download_extractor.py /tmp/huanyuxingqiu-57517.html
```
...
[
  {
    "href": "https://huanyuxingqiu.fun/goto?down=...",
...
.venv/bin/python category_link_collector.py \
  --base-url "https://huanyuxingqiu.fun/category/dou-yin-fan-cha/page/{page}" \
  --page-range 1:35 \
  --sleep 2 \
...
[
  {
    "source_page": 1,
    "title": "宫徵羽合集",
    "url": "https://huanyuxingqiu.fun/45790.html"
  }
]
```
...
```bash
.venv/bin/python post_chain_crawler.py --start "https://huanyuxingqiu.fun/3199.html" --max-pages 2
```
...
[
  {
    "url": "https://huanyuxingqiu.fun/3199.html",
    "title": "Booty徐莉芝 合集",
    "download_href": "https://huanyuxingqiu.fun/goto?down=...",
    "resolved_download_url": "https://share.feijipan.com/s/...?code=6666",
    "next_url": "https://huanyuxingqiu.fun/3203.html"
  }
]
```
...
hostname       -> rewrite_rules.py 里的 CONFIGURED_HOST_PATTERNS
```
